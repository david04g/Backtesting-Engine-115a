...
def increment_user_level(uid: str):
    try:
        ...
        user_level += 1
        response = supabase.table("user_level_progress").update({"level_progress": user_level}).eq("id", uid).execute()
        if "data=[]" in str(response):
            print("User does not exist")
            return None
        else:
...
def increment_user_lesson(uid: str):
    try:
        ...
        user_lesson += 1
        response = supabase.table("user_level_progress").update({"lesson_progress": user_lesson}).eq("id", uid).execute()
        if "data=[]" in str(response):
            print("User does not exist")
            return None
        else:
...
def increment_user_lesson_and_reset_user_level(uid: str):
    ...
    try:
        ...
        response = supabase.table("user_level_progress").update({"level_progress": 1, "lesson_progress": incremented_lesson}).eq("id", uid).execute()
